...
% Inicializa una lista Y de tama�o X con puras 'a' dentro
iniLista(0,Y):- Y=[],!.
iniLista(X,Y):- 
	Y = [a|Ys],
	X1 is X-1,
...
% Auxiliar de iterLista, para llevar un acumulador en la recursi�n
itLaux([X|[]],N,Ac):-
	esHlista(X,Ac,Ac1),
	N = Ac1,
	!.
...
% Indica si se hundieron todos los barcos, se satisface si el n�mero de posiciones iniciales de los barcos es igual al n�mero de h�s en el tablero
estadoFinal(T):- 
...
% actualizarLista(+L,+I,+E,Lf) :- Se satisface si Lf es una lista id�ntica a L,
%                 exceptuando en el elemento en la posici�n I, donde el elemento
%                 original es sustituido por E.
actualizarLista(L,I,E,Lf) :-
    actualizarListaAux(I,[],E,L,Lf).
% actualizarListaAux(I,H,E,T,Lf) :- Se satisface si Lf es una lista id�ntica a
%              L, exceptuando en el elemento en la posici�n I, donde el elemento
...
% obtenerElemento(+L,+I,E) :- Se satisface si E es el I-�simo elemento de la
...
% obtenerElemMatriz(M,F,C,E) :- Se satisface si E es el elemento en la F-�sima
%                               fila, en la C-�sima columna de la lista de lis-
